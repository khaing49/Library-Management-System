...
public class LibraryController {
    ...
    public ResponseEntity<List<Book>> getBooksByGenre(
        ...
        List<Book> books = (List<Book>) libraryService.getBooksByGenre(genre);
        logger.info("The books retrieved for genre "+genre);
        return new ResponseEntity<>(books, HttpStatus.OK);
    }
    //Get book by author/genre
    @GetMapping("/books/author/{author}")
    public ResponseEntity<List<Book>> checkGenreForAuthor(
            @PathVariable String author,
            @RequestParam(required = false) String genre) {
        List<Book> books = (List<Book>) libraryService.getBooksByAuthorAndGenre(author, genre);
        logger.info("The books retrieved for the author and genre "+author+" - " + genre);
        return new ResponseEntity<>(books, HttpStatus.OK);
    }
    //Get book by duedate
    @GetMapping("/books/dueondate")
    public ResponseEntity<List<Book>> getBooksDueOnDate(
